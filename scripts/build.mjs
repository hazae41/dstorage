...
for (const pathname of walkSync(`./out/v0`)) {
  if (pathname === `out/v0/start.html`)
    continue
  ...
  fs.copyFileSync(`./out/v0/start.html`, pathname)
}

fs.rmSync(`./out/v0/start.html`)
...
for (const pathname of walkSync(`./out/v0`)) {
  if (pathname === `out/v0/service_worker.latest.js`)
    continue
...
const original = fs.readFileSync(`./out/v0/service_worker.latest.js`, "utf8")
...
fs.writeFileSync(`./out/v0/service_worker.latest.js`, replaced, "utf8")
fs.writeFileSync(`./out/v0/service_worker.${version}.js`, replaced, "utf8")